...
var _ = Describe("Shoot-DNS-Service Tests", func() {
  ...
  "useNextGenerationController": true
		...
		createDelete = func(useNextGenerationController bool) {
			name := "local-wl"
			...
			if useNextGenerationController {
				providerConfig = rawExtensionNextGen
			}
			...
			_, err = controllerutil.CreateOrUpdate(ctx, shootClient.Client(), dnsEntry, func() error {
				...
				dnsEntry.Spec.DNSName = fmt.Sprintf("%t.shoot-dns-e2e-test.kind", useNextGenerationController)
