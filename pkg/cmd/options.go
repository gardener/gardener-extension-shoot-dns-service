...
func (o *DNSServiceOptions) AddFlags(fs *pflag.FlagSet) {
	...
	fs.Int32Var(&o.DefaultExternalProviderEntriesQuota, "default-external-provider-entries-quota", 0, "entries quota for the default 'external' DNSProvider (0 means no quota)")
...
func (o *DNSServiceOptions) Complete() error {
	...
	for _, mapping := range o.NextGenerationControllerZoneNameservers {
		...
		if parts[0] == "" || parts[1] == "" {
			return fmt.Errorf("zone or nameserver cannot be empty in nextgen-zone-to-nameserver mapping: %s", mapping)
		}
