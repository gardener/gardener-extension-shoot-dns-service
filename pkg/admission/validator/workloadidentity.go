...
func (wi *workloadIdentity) validateAWS(newObj, oldObj *securityv1alpha1.WorkloadIdentity) error {
	...
	if oldObj != nil {
		...
		errList := workloadidentity.ValidateAWSWorkloadIdentityConfigUpdate(oldConfig, newConfig, fieldPath)
		if len(errList) > 0 {
	...
	errList := workloadidentity.ValidateAWSWorkloadIdentityConfig(newConfig, fieldPath)
	if len(errList) > 0 {
		return fmt.Errorf("validation of target system's configuration failed: %w", errList.ToAggregate())
	}
	return nil
...
func (wi *workloadIdentity) validateAzure(newObj, oldObj *securityv1alpha1.WorkloadIdentity) error {
	...
	if oldObj != nil {
		...
		errList := workloadidentity.ValidateAzureWorkloadIdentityConfigUpdate(oldConfig, newConfig, fieldPath)
		if len(errList) > 0 {
			return fmt.Errorf("validation of target system's configuration failed: %w", errList.ToAggregate())
		}
		return nil
	...
	errList := workloadidentity.ValidateAzureWorkloadIdentityConfig(newConfig, fieldPath)
	if len(errList) > 0 {
		return fmt.Errorf("validation of target system's configuration failed: %w", errList.ToAggregate())
	}
	return nil
...
func (wi *workloadIdentity) validateGCP(newObj, oldObj *securityv1alpha1.WorkloadIdentity) error {
	...
	if oldObj != nil {
		...
		errList := gcp.ValidateWorkloadIdentityConfigUpdate(oldConfig, newConfig, fieldPath, wi.gcpConfig.AllowedTokenURLs, wi.gcpConfig.AllowedServiceAccountImpersonationURLRegExps)
		if len(errList) > 0 {
			return fmt.Errorf("validation of target system's configuration failed: %w", errList.ToAggregate())
		}
		return nil
	...
	errList := gcp.ValidateWorkloadIdentityConfig(newConfig, fieldPath, wi.gcpConfig.AllowedTokenURLs, wi.gcpConfig.AllowedServiceAccountImpersonationURLRegExps)
	if len(errList) > 0 {
		return fmt.Errorf("validation of target system's configuration failed: %w", errList.ToAggregate())
	}
	return nil
...
func awsConfigFromRawExtension(name string, providerConfig *runtime.RawExtension) (*workloadidentity.AWSWorkloadIdentityConfig, error) {
...
func azureConfigFromRawExtension(name string, providerConfig *runtime.RawExtension) (*workloadidentity.AzureWorkloadIdentityConfig, error) {
	cfg := &workloadidentity.AzureWorkloadIdentityConfig{}
...
func gcpConfigFromRawExtension(name string, providerConfig *runtime.RawExtension) (*gcp.WorkloadIdentityConfig, error) {
