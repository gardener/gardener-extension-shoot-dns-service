...
func validateProviderWorkloadIdentity(workloadIdentityName, providerType string, path, secretChild *field.Path, getter ResourceGetter, allErrs *field.ErrorList) {
	if providerType != "" && getter != nil {
		...
		case "aws-route53":
			...
			var providerConfig workloadidentity.AWSWorkloadIdentityConfig
			...
			errList := workloadidentity.ValidateAWSWorkloadIdentityConfig(&providerConfig, secretChild.Child("ref"))
			if len(errList) > 0 {
		...
		case "google-clouddns":
			...
			var providerConfig gcp.WorkloadIdentityConfig
			if err := yaml.Unmarshal(workloadIdentity.Spec.TargetSystem.ProviderConfig.Raw, &providerConfig); err != nil {
				*allErrs = append(*allErrs, field.Invalid(secretChild.Child("ref"), workloadIdentityName,
					fmt.Sprintf("failed to unmarshal the WorkloadIdentity providerConfig: %s", err)))
				return
			...
			errList := gcp.ValidateWorkloadIdentityConfig(&providerConfig, secretChild.Child("ref"), gcpConfig.AllowedTokenURLs, gcpConfig.AllowedServiceAccountImpersonationURLRegExps)
			if len(errList) > 0 {
				*allErrs = append(*allErrs, errList...)
				return
			}
		case "azure-dns", "azure-private-dns":
			...
			var providerConfig workloadidentity.AzureWorkloadIdentityConfig
			if err := yaml.Unmarshal(workloadIdentity.Spec.TargetSystem.ProviderConfig.Raw, &providerConfig); err != nil {
				*allErrs = append(*allErrs, field.Invalid(secretChild.Child("ref"), workloadIdentityName,
					fmt.Sprintf("failed to unmarshal the WorkloadIdentity providerConfig: %s", err)))
				return
			...
			errList := workloadidentity.ValidateAzureWorkloadIdentityConfig(&providerConfig, secretChild.Child("ref"))
			if len(errList) > 0 {
				*allErrs = append(*allErrs, errList...)
				return
			}
