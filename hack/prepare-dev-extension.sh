...
if ! SERVICE_IP_ADDRESS=$(kubectl -n knot-dns get secret knot-dns-secret -ojsonpath='{.data.Server}' 2>/dev/null | base64 -d); then
    echo "Error: Failed to retrieve knot-dns-secret" >&2
    exit 1
fi
    ...
    extension:
      values:
        ...
        nextGenerationController:
          zoneToNameserver:
            "shoot-dns-e2e-test.kind.": ${SERVICE_IP_ADDRESS}
EOF